...
neural = new NN(layers, epsilon, lambda, alph, regression);
diameter = 80;
count = 0
...
function reset() {
  ...
  alph       = document.getElementById("alpha").value.split(" ").map(Number);
...
// calculate each neuron position based on given width and height
...
// create node class to track network neurons
function node(x,y) {
...
function draw() {
  ...
  // createa and display a network
  nodes = create_network(0,10,750,465,layers,true,act);
  ...
  if (count % 10 === 0) {
    ...
    loss.innerHTML = "Loss: " + Math.round(neural.err*10000)/10000;

    dcount = document.getElementById("dcount");
    dcount.innerHTML = "Iterations: " + count*iters;
  }
  count++;
}